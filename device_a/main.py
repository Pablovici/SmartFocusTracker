...
def _alert_overlay(text):
    """Affiche l'overlay d'alerte avec le texte wrap."""
...
def _play_alert_wav(wav_path):
    """Joue un fichier WAV pre-genere (pas de requete reseau)."""
...
def _log_alert_bq(alert_type, message):
    """Envoie un log d'alerte au middleware pour insertion BigQuery."""
    try:
...
def speak_alert_cached(wav_file, display_text):
    """Alerte avec WAV pre-genere — instantane, pas de reseau."""
    global _main_needs_full_redraw
    _alert_overlay(display_text)
    bip(900)
...
def speak_alert(text):
    """Alerte vocale dynamique via TTS (pour messages avec donnees variables)."""
    global _main_needs_full_redraw
    _alert_overlay(text)
    bip(900)
    gc.collect()
...
def check_alerts():
    ...
    # Alerte pause — WAV pre-genere, instantane
    ...
    # Alerte qualite de l'air — WAV pre-genere, instantane
